...
+ LNX_DrumSynth {
	...
	*thisHeight {^408+20}
	...
	// Themes
		...
		gui[\onOffTheme4]=( \font_		: Font("Helvetica-Bold", 10),
	...
	// The scroll views
		...
		gui[\compositeView] = MVC_RoundedComView(window,
				...
				.color_(\border,  Color(59/108,505/692,65/103)  )
				.color_(\border2, Color(0,9/77,1/103))
				.color_(\background, Color(59/77,9/11,43/54) );
	
	
	
		gui[\masterView]= MVC_CompositeView(gui[\compositeView], Rect(10,179,650,225))
			.hasBorder_(false);
			
		gui[\drumViews]=nil!5;
			
		gui[\drumColours].do({|c,j|
			gui[\drumViews][j]= MVC_CompositeView(gui[\masterView], Rect(1+(j*120),1,115,213))
				.color_(\background,c);
		});
	
		gui[\tabView]=MVC_TabbedView(gui[\compositeView],Rect(10, 32, bounds.width-40+15-20, 140))
			.labels_(["Bass","Snare","Clap","Tom","Hi Hat"])
			.tabCurve_(4)
			.labelColors_(gui[\drumColours])
			.unfocusedColors_(gui[\drumColours])
			.backgrounds_(gui[\drumColours])
	...
	// widgets
		...
		// 0.solo
		MVC_OnOffView(models[0],gui[\compositeView]     ,Rect( 30,10,17,17),gui[\soloTheme  ]);
		// 1.on/off
		MVC_OnOffView(models[1],gui[\compositeView]     ,Rect( 10,10,17,17),gui[\onOffTheme2]);
		
		// MIDI Settings
 		MVC_FlatButton(gui[\compositeView],Rect(56, 9, 43, 19),"MIDI")
			.rounded_(true)
			.canFocus_(false)
			.shadow_(true)
		...
		// MIDI Control
		MVC_FlatButton(gui[\compositeView],Rect(108, 9, 43, 19),"Cntrl")
			.rounded_(true)
			.canFocus_(false)
			.shadow_(true)
			.color_(\up,Color(223/375,2/3,10665/19997) )
		...
		// 119.lock seq
		MVC_OnOffView(models[119],gui[\compositeView],Rect(158,10,54,16))
			.font_(Font("Helvetica",10))
			.color_(\on,gui[\highlightColours][3])
			.color_(\off,gui[\drumColours][3]);	
			
		// the preset interface
		presetView=MVC_PresetMenuInterface(gui[\compositeView],220@9,155,
			gui[\drumColours][4],gui[\highlightColours][4],gui[\highlightColours][4]/1.5);
		this.attachActionsToPresetGUI;
		
		
		// 34.show sequencer
		MVC_OnOffView(models[26],gui[\compositeView]    ,Rect(473, 10,30,18),gui[\onOffTheme ]);
		
		// 132.master send channels
		MVC_PopUpMenu3(models[132],gui[\compositeView]    ,Rect(514,10,70,18),gui[\menuTheme  ]);		
		
		
		// 25.output channels
		MVC_PopUpMenu3(models[25],gui[\compositeView]    ,Rect(594,10,70,18),gui[\menuTheme  ]);
	

	
		// 2.amp
		MVC_SmoothSlider(models[2],gui[\masterView], Rect(614, 35, 20, 63),gui[\sliderTheme2])
			.color_(\knob,gui[\highlightColours][4]);
		
		// 11.bp scale (-1,1)
		MVC_MyKnob(models[11],gui[\masterView],Rect(614,10,20,20),gui[\knob2Theme1])
			.color_(\on, gui[\highlightColours][4])
			.color_(\pin, gui[\highlightColours][4]);
			
		// 130.pan
		MVC_MyKnob(models[130],gui[\masterView],Rect(614,156,20,20),gui[\knob2Theme1])
			.color_(\on, gui[\highlightColours][4])
			.color_(\pin, gui[\highlightColours][4]);
			

		// 133.master send
		MVC_MyKnob(models[133],gui[\masterView],Rect(614,191,20,20),gui[\knob2Theme1])
			.color_(\on, gui[\highlightColours][4])
			.color_(\pin, gui[\highlightColours][4]);

			
		// 117.dur
		MVC_MyKnob(models[117],gui[\masterView],Rect(614,118,20,20),gui[\knob2Theme1])
			.color_(\on, gui[\highlightColours][4])
			.color_(\pin, gui[\highlightColours][4]);
	...
	// all drums
		...
		gui[\drumViews].do{|view,y|
			// onOff
			MVC_OnOffView(models[120+y],view,Rect(5,5,37,17),gui[\onOffTheme4])
				.color_(\on,gui[\highlightColours][y]);
			// BP
			MVC_OnOffView(models[52+y],view,Rect(89,6,17,15),gui[\onOffTheme4])
				.color_(\on,gui[\highlightColours][y]);
			// Amp
			...
			// 27-31.out channel
			MVC_PopUpMenu3(models[27+y],view,Rect(37,152,70,17),gui[\menuTheme2])
				.color_(\background,gui[\drumColours][y]/3)
		...
		channels.do{|y|
			...
			tab=gui[\tabView].mvcTab(y);
			
			defaultSteps.do{|x|
				var c,col;
				c=(x%(sP[y][4])).map(0,sP[y][4],1,0.4);
				col=gui[\sliderColours][y];
				seqMVCViews[y][x]= MVC_FlatSlider(tab,seqModels[y][x],
										Rect(71+(x*18),20,17,90))
				.rounded_(true)
			...
			// 4.ruler	
	 	 	MVC_NumberBox(spModels[y][4],tab,Rect(40,6,23,17),theme);
	 	  	MVC_RulerView(spModels[y][4],tab,Rect(70,2,(defaultSteps*18),15))
	 	   		.label_(nil).steps_(defaultSteps)
	 	   		.color_(\on,gui[\highlightColours][y]+0.4)
				.color_(\background,gui[\drumColours][y]/2+0.2);
			//3.steps
			...
			// 6.speed
		  	...
		  	// pos
		  	MVC_PosView(posModels[y],tab,Rect(70, 112,(defaultSteps*18), 5))
				.color_(\on,gui[\highlightColours][y])
				.color_(\background,gui[\drumColours][y]/2);
