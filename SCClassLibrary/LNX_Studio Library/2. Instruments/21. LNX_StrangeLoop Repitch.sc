...
+ LNX_StrangeLoop {
	...
	pitch_clockIn3{|instBeat,absTime3,latency,beat|
		...
		if (relaunch or:{beat % length==0}) {
			...
			bufferL			= sample.buffer.bufnumPlayback(0);          	// this only comes from LNX_BufferArray
			bufferR			= sample.buffer.bufnumPlayback(1) ? bufferL; 	// this only comes from LNX_BufferArray
