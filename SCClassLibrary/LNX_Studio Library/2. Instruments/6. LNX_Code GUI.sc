	...
	createWidgets{
		...
		systemViews[\sendAmp] =
			MVC_MyKnob3(models[10],
				gui[\userGUIScrollView],Rect(568,210,26,26),gui[\knobTheme2])
		...
		// 15. inAmp
		systemViews[\inAmp] =
			MVC_MyKnob3(models[15],
				gui[\userGUIScrollView],Rect(568,268,26,26),gui[\knobTheme2])
			.boundsAction_{|view,rect|
		...
		window.helpAction_{
			...
			if (gui.notNil) {
				if (p[13]==3) {
					string=gui[\code].selectedString.split($.)[0];
					if (string.asSymbol.asClass.isNil) {
						"How to program LNX_Studio".help
					}{
						string.help
