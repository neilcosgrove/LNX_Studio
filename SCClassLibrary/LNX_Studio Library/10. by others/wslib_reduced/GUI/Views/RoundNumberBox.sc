RoundNumberBox : RoundView {
	...
	var <>postfix="", <>postfixFunc, <>prefix="";
...
// 								{ Pen.setShadow( *innerShadow.asCollection );  };
			...
			Pen.use({
				...
				if (postfixFunc.notNil) { prefix=postfixFunc.value(value) };


				if( autoScale )
				{ Pen.stringCenteredIn( prefix++string++postfix, stringRect.moveBy(0,1) ); }
				{
				Pen.perform(
					( left: \stringLeftJustIn, right: \stringRightJustIn,
						center: \stringCenteredIn, middle: \stringCenteredIn )[ align ]
						? \stringLeftJustIn, prefix++string++postfix, stringRect.moveBy(0,1) );
				}
