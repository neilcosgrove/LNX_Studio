...
//			{ calcTabWidth = tabWidth };
		...
		if (scroll.isCollection) {
			scroll[index].if{container = ScrollView.new(view).resize_(5)}
			{container = CompositeView.new(view,view.bounds).resize_(5)}; //bounds are set

		}{

			scroll.if{container = ScrollView.new(view).resize_(5)}
			{container = CompositeView.new(view,view.bounds).resize_(5)}; //bounds are set later
		};
