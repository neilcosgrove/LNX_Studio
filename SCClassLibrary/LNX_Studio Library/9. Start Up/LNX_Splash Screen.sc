...
LNX_SplashScreen{
	...
	classvar window, backgroundPath="screen shot 1.3.png", gui;

	*init {|studio,forceOpen=false|
...

		{
			if (("SplashScreen".loadPref.isNil || forceOpen) and: {(window.isNil) or: {window.isClosed}} ) {
				...
				image=Image.open(Platform.lnxResourceDir +/+ backgroundPath);
				w=image.width/1.5;
				h=image.height/1.5;
				...
				image2=Image.open(Platform.lnxResourceDir +/+ "lnx.png");
