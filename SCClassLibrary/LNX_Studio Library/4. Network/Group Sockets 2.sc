...
//	}
//	
//	join{ ("LNX_LANGroup started.").postln; }
//	
...
//						tryAddr.sendBundle(nil, [\m,0,uid]++[\scan]++
...
//		};
//		
//		otherAddrs = lanAddrs.difference(myAddrs);
//	}
//	
...
//			msg=msg.drop(msg[0]+1); // and drop junk
//			inUID=msg[0];           // the incoming user ID
//			msg=msg.drop(1);        // and drop it from the message
//			
//			// the below test is a saftey net to stop lnx sending messages to itself.
...
//	sendBundle {|time,msg|
//		
//		// junk to change message. see if this helps on-line. prob not!
//		var random=[5.rand];
//		random = random ++ (random[0].collect{|r| [(2**15).rand,String.rand(5.rand)].choose});
//		
//		if ((verbose)and:{msg[0]!=\broadcastProfile}and:{msg[4]!=\rcs}) {
//			"OUT: ".post;
//			msg.postln
//		};
//		
//		otherAddrs.do{|addr| addr.sendBundle(time,[\m]++random++[uid]++msg); };
//		
//	}
//
//}
...
////	sendMsg { arg ... msg;
////		msg[0] = this.formatSymbol(msg[0]);
////		if (verbose) {("Sending..."+(msg.asString)).postln;};
////		netAddr.sendMsg(*msg);
////	}
////	
...
////		}
////	
////	sendBundle { arg time ... msg;
////		if (pid.notNil) {
////			msg.do({|msg|
////				if ((verbose)and:{msg[0]!=\broadcastProfile}) {
////					if ((msg.size>=5)and:{msg[4]!=\rcs}) {
////						msg.postln;
////					};
////				};
////				msg[0] = this.formatSymbol(msg[0]);
